...
class Agent(object):
    def __init__(self):
        ...
        model_path_actor = os.path.join(config.MODEL_SAVE_DIR, f"{load_env_name}_actor{load_timestep}.pth")
        model_path_critic = os.path.join(config.MODEL_SAVE_DIR, f"{load_env_name}_critic{load_timestep}.pth")
        ...
        try:
            ...
            print(f"Warning: Model file not found in {config.MODEL_SAVE_DIR} for {load_env_name} at timestep {load_timestep}.")
            print("Falling back to random action selection.")
        except Exception as e:
            print(f"Warning: An error occurred while loading the model: {e}")
            print("Falling back to random action selection.")

        # Models are set to evaluation mode if loaded successfully
    ...
    def act(self, observation):
        ...
        return action_env  # Should be numpy array, e.g., np.array([value]) for Pendulum